...
// 🌟 1. นำเข้า useTranslation จาก react-i18next
import { useTranslation } from "react-i18next";
// 🌟 2. นำเข้าไฟล์ตั้งค่า i18n (อย่าลืมสร้างไฟล์ i18n.ts ตามที่เราคุยกันไว้ด้วยนะครับ)
import "@/libs/i18n"; 

export default function NavBar() {
  const router = useRouter();
  
  // 🌟 3. เรียกใช้งาน t (สำหรับแปลคำศัพท์) และ i18n (สำหรับเปลี่ยนภาษา)
  const { t, i18n } = useTranslation();
  
  const [user, setUser] = useState<GetAuthAccountResponse["data"] | null>(null);

  // 🌟 4. ฟังก์ชันสำหรับกดสลับภาษา TH <-> EN
    ...
    <nav className="fixed top-0 left-0 right-0 flex items-center justify-between h-12.5 z-30 px-6 py-4 bg-[#111111] text-white font-salsa border-b-2 border-[#fdd835]">
      ...
      <div className="flex items-center space-x-6 text-sm font-medium">
        ...
        >
          ...
          {/* 🌟 5. เปลี่ยนตัวหนังสือเป็น t() */}
          {t("nav.help")}
        </Link>
        
        <Link href="/" className="hover:text-yellow-400">
          {/* 🌟 6. เปลี่ยนตัวหนังสือเป็น t() */}
          {t("nav.home")}
        </Link>

        {/* 🌟 7. ปุ่มเปลี่ยนภาษา (ผูกฟังก์ชัน onClick เข้าไป) */}
        <button 
        ...
        >
          ...
          {/* 🌟 8. แสดงตัวย่อของภาษาถัดไปที่จะเปลี่ยน เช่น ถ้าตอนนี้เป็น en ปุ่มจะขึ้น TH เพื่อให้กดเปลี่ยน */}
